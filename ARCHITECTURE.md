...
- **OpenCode is a second host, not a peer:** OpenCode gets the RTK shell rewrite, AGENTS.md guidance, and prompt commands in `~/.config/opencode/commands/`. Its commands are prompt templates, not runtime toggles — an OpenCode command sends text to the model and cannot flip state the way OMP's `pi.registerCommand` does. The live Combo state bridge and the status bar stay OMP-only because OpenCode has no equivalent of the in-process bridge.
