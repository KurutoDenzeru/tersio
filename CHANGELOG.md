## Unreleased
 ...
 - **Fixes a stored agent choice hiding hosts the user already has.** With `~/.tersio/agents.json` holding `["omp"]`, `tersio install --yes` printed `Hosts: Oh My Pi (OMP)` and wrote nothing for OpenCode — even with `~/.config/opencode` present and OpenCode in daily use. No plugin, no guidance block, no command files, and no warning. The non-interactive path returned the stored list verbatim while the interactive menu already unioned it with detection, so a host present on disk was only ever installed if it had been ticked in the menu. `--yes`, CI, and pipes now take the same union. An explicit `--agent` flag is still authoritative and is never widened.
 - That bug was pinned by a test. `a stored choice wins over detection on later runs` asserted the exact wrong behavior, so fixing the code broke the test that defended it; the test now asserts the intended rule instead. Reverting the fix fails three tests.
