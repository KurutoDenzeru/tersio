## Unreleased
 ...
 - Pi wiring now checks the binary advertises `--agent pi` before running it. rtk 0.49 added that agent; an older binary ignores the flag and writes an OMP extension instead, which looked like success while leaving Pi unwired. A reinstall also no longer stalls on rtk's "overwrite the non-stock extension?" prompt — the existing file is ours, so it is refreshed non-interactively.
