...
export function Brandmark({ model, small }: { model: string; small?: boolean }) {
  ...
  const cls = `brandmark${small ? " sm" : ""}`;
  if (model === "openai" || v.slug === "openai") {
    return (
      <span className={cls} style={{ background: v.color }} title={v.name}>
        {OPENAI_SVG}
      </span>
    );
  }
  ...
  return (
    <span className={cls} style={{ background: v.color }} title={v.name}>
      <img src={`https://cdn.simpleicons.org/${v.slug}/white`} alt="" loading="lazy" onError={() => setFailed(true)} />
